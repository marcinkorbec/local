...
import { modalTypes } from './DepositoryTableData';
import ModalContentMove from './ModalContentMove';

const LINK = 'local/books/depository';
const NAME = 'depository';

const DepositoryTable = () => {
  ...
  const deleteItem = async (id, booksIds) => {
    try {
      await Promise.all(booksIds?.map(bookId => api.putRemoveItem(id, bookId)));
  ...
  const handleOpenModal = (row, fName) => {
    ...
    switch (fName) {
      case 'move':
        ...
        break;
      case 'returnItem':
        setModalType(modalTypes.returnItem);
        setSubmit(modalTypes.returnItem);
        break;
      case 'removeItem':
  ...
  return (
    <>
      <Table
        ...
        noFilterColumns={['id', 'isReturned', 'rentFrom', 'rentTo']}
      />
...
export default DepositoryTable;
