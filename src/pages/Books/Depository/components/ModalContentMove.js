...
const ModalContentMove = ({
  ...
  const handleSubmit = async e => {
    ...
    try {
      if (returnItem) {
        await api.putReturnItem(row.id, fd);
        toast.success(`Przedmiot ${row.name} został zwrócony`, OPTIONS);
      } else {
        await api.putMoveItem(row.id, selectedBook.value, '', fd);
        toast.success(
      ...
      toast.error(
        `${
          returnItem
            ? 'Wystąpił błąd.'
            : 'Wystąpił błąd podczas przenoszenia przedmiotu.'
        } ${err?.response?.data}`,
        OPTIONS
      );
    }
  ...
  return (
    <FormModal onSubmit={handleSubmit}>
      <p>{returnItem ? 'Zwróć przedmiot:' : 'Przenieś przedmiot:'}</p>
      <p>
        ID: <span>{row?.id}</span>
      </p>
      <p>
